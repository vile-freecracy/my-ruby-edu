...
class GraphqlController < ApplicationController
  ...
  def execute
    ...
    context = {
      ...
      # expose method to check token revocation
      token_revoked?: ->(jti) { RevokedToken.exists?(jti: jti) }
    }
    result = MyappSchema.execute(query, variables: variables, context: context, operation_name: operation_name)
    render json: result
  rescue JWT::DecodeError => e
    render json: { errors: [{ message: "Invalid token: #{e.message}" }] }, status: :unauthorized
  end
  ...
  def current_user_from_token
    ...
    #   raise JWT::DecodeError, 'Token revoked'
    # end
